...
$category_id = '';
    ...
    $oldProduct = Product::getOnceProductByID($pdo, $_GET["id"]);
    $name = $oldProduct["name"];
    $price = $oldProduct["price"];
    $desc = $oldProduct["description"];
    $image = $oldProduct["image"];
    $category_id = $oldProduct["cat_id"];
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(!$nameErr && !$priceErr && !$category_idErr) {
            $newPro = new Product();
            $newPro->id = $_GET["id"];
            $newPro->name = $name;
            $newPro->price = $price;
            ...
            $conn = new Database();
            $pdo = $conn->getConnect();
            Product::updateProduct($pdo, $newPro);
            //var_dump($newPro);
            header("location: admin-page.php");
        }
    }
...
<body>
    ...
    <main>
        ...
        <img src="../Image/<?= $oldProduct["image"]?>" width="100px"></img>          
        </div>

        <div class="mb-3">
            <?php $selectedCategoryId = $oldProduct["cat_id"] ?>
            ...
            <select id="selcect-category" name="category_id" class="mx-2" style="height: 30px;">
                <?php foreach ($categories as $category) : ?>
                    <option 
                        id="<?=$category["cat_id"]?>" name="category_id" value="<?=$category["cat_id"]?>" 
                        <?= ($category["cat_id"] == $selectedCategoryId) ? 'selected' : '' ?> >
                        <?= $category["name"] ?>
                    </option>
                <?php endforeach ?>
            </select>

<?php 
    ...
    if (isset($_GET["cat_id"])) {
        ...
        //$products = Product::getProductsByCatID($pdo, $cat_id);

        $paginator = new Paginator(4);
        $current_page = $paginator->getCurrentPage();
        $start = $paginator->getStartIndex($current_page);
        $products = $paginator->getDataByCatID($pdo, $start, $cat_id);
        $total_pages = $paginator->getTotalPagesForCatID($pdo, $cat_id);
    }
    else {
    ...
    if($_GET["cat_id"] == $hamsterCat_id) {
        $nameCategory = "Chuột Hamster";
    } else if($_GET["cat_id"] == $proHamsterCat_id) {
        $nameCategory = "Sản Phẩm Hamster";
    }
    else {
        $nameCategory = Category::getNameCategoryByCatID($pdo, $_GET["cat_id"]);
    }
...
<div class="container-fluid mx-0 mb-0" style="background-color: white; margin-top: 56px;">
    ...
    <div class="contents" style="display:flex; justify-content: center;">
        ...
        <div class="all-product" style="flex: 4;">
            <h3 class="text text-danger text-center m-3">Danh Sách <?= $nameCategory?> </h3>
                    ...
                    <div class="card" style="width: 16rem; margin: 10px 10px;">
                        ...
                        <div class="card-body" style="display: flex; justify-content:space-between; flex-direction: column;">
                            ...
                            <div class="pay-button-group mt-3" style="display: flex; justify-content: center;">
                                ...
                                <a href="ProductsByCategory.php?cat_id=<?=$cat_id?>&page=<?=$current_page?>&action=addcart&pro_id=<?=$product->id?>" class="btn btn-outline-danger mx-1 px-3" style="display: flex; justify-content: center; align-items: center;">
                ...
                ?>
                    <div class="container">
                        <ul class="pagination justify-content-center">
                            <!-- Previous page-->
                            <li class="page-item">
                                <a class="page-link" href="?cat_id=<?= $cat_id?>&page=<?= $prev_page ?>">Previous</a>
                            </li>
                            ...
                            <!-- Next page-->
                            <li class="page-item">
                                <a class="page-link" href="?cat_id=<?= $cat_id?>&page=<?= $next_page ?>">Next</a>
                            </li>
                        </ul>
